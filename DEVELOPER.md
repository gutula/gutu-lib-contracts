...
- You need **shared contracts**, **type utilities**, **cross-package consistency** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/contracts` instead of reaching into app-specific internals.
...
| `framework/libraries/contracts` | Nested publishable library package. |
| `framework/libraries/contracts/src` | Public runtime source and exported modules. |
| `framework/libraries/contracts/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `z` |
...
- Hosts should import from `@platform/contracts`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/contracts"]
...
- Compose through documented modules such as `@platform/admin-contracts`, `@platform/kernel`, `@platform/schema`, `zod`.
